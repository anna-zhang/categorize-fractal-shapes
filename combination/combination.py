...
def shape_exists(img_fp, white_threshold): 
    assert os.path.isfile(img_fp), 'file \'{0}\' does not exist'.format(img_fp) # make sure reference image exists at the filepath specified
    img = cv2.imread(img_fp, cv2.IMREAD_GRAYSCALE) # read reference image as an array of grayscale values so only getting one value per pixel instead of values for three color channels per pixel
    img = img / 255.0
...
def main():
    ...
    final_output_path = r'final_output' # final output folder path
    ...
    if program_mode == "-specific": # compare two specific images using just histogram comparison
        ...
        for i in range(num_frames):
            if images[i]["categorized"] == False: # go to the next uncategorized image and set that as the reference image for a new shape category
                ...
                histogram_categories[str(num_categories)] = [] # array of all image numbers in this category
                histogram_categories[str(num_categories)].append(reference_img_fp) # save reference image filepath
                ...
                for j in range(i + 1, num_frames): 
                    if images[j]["categorized"] == False:
                        ...
                        if (initial_match.match):
                            ...
                            histogram_categories[str(num_categories)].append(uncategorized_img_fp) # save image filepath for this new image added to the category
        ...
        shapeless_category_path = r'final_output/category.' + '{:0>3}'.format(0) # output category folder path for shapeless images
        ...
        for category, image_list in histogram_categories.items():
            for image in image_list:
                categorizations[image] = {}
                categorizations[image]["categorized"] = False # initialize all images in the subcategory to uncategorized
                categorizations[image]["category_num"] = -1 # initialize category number to -1 when uncategorized
                categorizations[image]["error"] = False # initialize to no errors in categorization

                # remove images with super few white pixels from EMD consideration and place them all in category.000
                # shape_exists
                if (shape_exists(image, white_threshold) == False):
                    # copy shapeless image into shapeless category
                    categorized_output_fp = shapeless_category_path + '/' + image[-16:-4] + '.png'
                    shapeless_image = cv2.resize(cv2.imread(image, cv2.IMREAD_GRAYSCALE), (x_res, y_res)) # read  image as an array of grayscale values so only getting one value per pixel instead of values for three color channels per pixel
                    cv2.imwrite(categorized_output_fp, shapeless_image) 
                    categorizations[image]["categorized"] = True # categorized in shapeless category
                    categorizations[image]["category_num"] = 0 # in shapeless category 

            for i in range(len(image_list)):
                image = image_list[i] # get the filepath of an image in this shape category
                print("IMAGE: " + str(image))
                if (categorizations[image]["categorized"] == False): # go to the next uncategorized image in this shape category and set that as the reference image for a new shape category
                    # read in reference image file
                    ...
                    new_category_path = r'final_output/category.' + '{:0>3}'.format(num_final_categories) # output category folder path
                    if not os.path.exists(new_category_path):
                        os.makedirs(new_category_path)
                    ...
                    categorizations[image]["categorized"] = True # remember that this image has been categorized
                    categorizations[image]["category_num"] = num_final_categories # remember the category this image is a part of
            
                    for j in range(i, len(image_list)): 
                        # read in uncategorized image file
                        uncategorized_img_fp = image_list[j] # uncategorized image filepath
                        if (categorizations[uncategorized_img_fp]["categorized"] == False): # go to the next uncategorized image in this shape category
                            assert os.path.isfile(uncategorized_img_fp), 'file \'{0}\' does not exist'.format(uncategorized_img_fp) # make sure the uncategorized image exists at the filepath specified
                            uncategorized_img = cv2.resize(cv2.imread(uncategorized_img_fp, cv2.IMREAD_GRAYSCALE), (x_res, y_res)) # read uncategorized image as an array of grayscale values so only getting one value per pixel instead of values for three color channels per pixel
                            ...
                            if (shape_match.error):
                                categorizations[uncategorized_img_fp]["error"] = True
                                categorizations[uncategorized_img_fp]["categorized"] = True # remember that this image has been categorized
                                continue
                            # compare the shapes in the two images
                            if (shape_match.match):
                                ...
                                categorizations[uncategorized_img_fp]["categorized"] = True # remember that this image has been categorized
                                categorizations[uncategorized_img_fp]["category_num"] = num_final_categories # save the category number the image was categorized in
        ...
        info_file.write("\n With EMD on histogram category results: \n") # spacer and header for summary info        
        info_file.write(str(categorizations)) 
